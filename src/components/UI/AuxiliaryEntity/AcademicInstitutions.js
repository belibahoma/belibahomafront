...
export default class AcademicInstitutions extends Component {
  ...
  componentDidMount() {
    axios
      .get(`${config.get("serverAddress")}/api/institutes`)
      .then(response => {
  ...
  hadleDelete = params => {
    axios
      ...
      .then(response => {
        const institutes = this.state.institutions.filter(
          m => m._id !== params._id
        );
  ...
  handleSubmitEdit = params => {
    ...
    axios
      .put(
        `${config.get("serverAddress")}/api/institutes/` +
          this.state.instituteId,
        dataToPut
      )
      .then(res => {
        const institutes = this.state.institutions.filter(
          m => m._id !== this.state.instituteId
        );
