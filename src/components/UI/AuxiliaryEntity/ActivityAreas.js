...
export default class ActivityAreas extends Component {
    ...
    componentDidMount() {
        axios.get(`${config.get("serverAddress")}/api/areas`)
            .then(response => {
    ...
    handleSubmitEdit = params => {
        ...
        axios
            .put(`${config.get("serverAddress")}/api/areas/` + this.state.areaId, dataToPut)
            .then(res => {
                const _areas = this.state.areas.filter(m => m._id !== this.state.areaId);
        ...
        return (
            <div className="container" >
                <Modal show={this.state.isShowing} onHide={this.toggleModal_addItem} style={{ textAlign: 'right' }}>
                    <Modal.Header   >
                        <Modal.Title style={{ margin: '0 auto ' }}>הוספת איזור פעילות</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form >
                ...
                <Modal show={this.state.editIsShoing} onHide={this.toggleModal_editItem} style={{ textAlign: 'right' }}>
                    <Modal.Header >
                        <Modal.Title style={{ margin: ' 0 auto ' }}>עריכת איזור פעילות</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <Form >
                            <Form.Group controlId="formGroupareaName">
                ...
                <button className='btn btn-outline-primary' onClick={this.toggleModal_addItem} style={{ float: 'left' }}>הוסף איזור פעילות</button>
                <MultyTableGeneric ColumnNames={this.state.TableColumns} data={this.state.areas} hadleDelete={this.hadleDelete} hadleEdit={this.hadleEdit} />
            </div>
        );
    }
