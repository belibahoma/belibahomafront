...
export default class Tutors extends Component {
        ...
        this.state = {
            TableColumns: [{
                ...
                formatter: (value, row, index) => {
                    return value === "active" ? "פעיל" : value === "not active" ? "לא פעיל" : value === "graduate" ? "בוגר " : value === "candidate" ? "מועמד" : "";
                },
            }, {
    ...
    hadleDelete = (params) => {
        ...
        axios
            ...
            .then(response => {
                const _tutorList = this.state.tutorList.filter(m => m._id !== params._id)
    ...
    render() {
        ...
        return (
            <div className="ml-5 mr-5 " >
                <Modal size="lg" show={this.state.editIsShoing} onHide={this.toggleModal_editItem} style={{ textAlign: 'right' }}>
                    <Modal.Header   >
                ...
                <MultyTableGeneric ColumnNames={this.state.TableColumns} data={this.state.tutorList} hadleDelete={this.hadleDelete} hadleEdit={this.hadleEdit} />
            </div>
        );
    };
